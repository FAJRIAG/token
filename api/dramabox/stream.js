...
module.exports = async function handler(req, res) {
  ...
  const bookId = ((req.query?.bookId || req.query?.bookid || "") + "").trim();
  ...
  const upstream = `${U.BASE}/dramabox/stream?bookId=${encodeURIComponent(bookId)}&bookid=${encodeURIComponent(bookId)}&episode=${episode}`;
  ...
  // Fallback anti-502: demo HLS agar player tetap jalan
